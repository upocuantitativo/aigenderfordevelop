...
# -*- coding: utf-8 -*-
"""
Final Dual Target Analysis with ROBUST model selection
G_GPD_PCAP_SLOPE + G_Score-Trading across borders
Uses composite score (0.6*R² + 0.4*CV if CV>0) to select best model
"""
...
def get_composite_score(r2, cv):
    """Composite score prioritizing positive CV"""
    if cv < 0:
        return r2 * 0.3  # Heavily penalized
    return 0.6 * max(0, r2) + 0.4 * cv

print("="*80)
print("FINAL DUAL TARGET ANALYSIS (ROBUST)")
...
for target_name, target_col in targets.items():
    ...
    models['Random Forest'] = {
        ...
        'cv': cv_rf,
        'composite_score': get_composite_score(r2_score(y_test, y_pred_rf), cv_rf),
    ...
    models['XGBoost'] = {
        ...
        'cv': cv_xgb,
        'composite_score': get_composite_score(r2_score(y_test, y_pred_xgb), cv_xgb)
    }
    ...
    models['Gradient Boosting'] = {
        ...
        'cv': cv_gb,
        'composite_score': get_composite_score(r2_score(y_test, y_pred_gb), cv_gb)
    }
    ...
    models['Neural Network'] = {
        ...
        'cv': cv_nn,
        'composite_score': get_composite_score(r2_score(y_test, y_pred_nn), cv_nn)
    }

    # Rank by COMPOSITE SCORE (prioritizes positive CV)
    ranked = sorted(models.items(), key=lambda x: x[1]['composite_score'], reverse=True)
    top_3 = ranked[:3]
    ...
    for i, (name, res) in enumerate(ranked, 1):
        ...
        print(f"  {i}. {name:20s} R2={res['r2']:.4f} CV={res['cv']:.4f} ({cv_status}) Score={res['composite_score']:.4f}")
...
# Save results
with open('resultados/modelos/final_dual_target_results_robust.pkl', 'wb') as f:
    pickle.dump(all_results, f)

print("\n" + "="*80)
print("SUMMARY (ROBUST MODEL SELECTION)")
print("="*80)
...
for target_name, res in all_results.items():
    ...
    print(f"  Composite Score: {best_res['composite_score']:.4f}")
    print(f"  RMSE: {best_res['rmse']:.2f}")
    print(f"  Bootstrap CI: [{res['validation']['bootstrap_ci'][0]:.3f}, {res['validation']['bootstrap_ci'][1]:.3f}]")

print(f"\nResults saved: resultados/modelos/final_dual_target_results_robust.pkl")
print("\nNote: Models selected using composite score = 0.6*R² + 0.4*CV (if CV>0)")
print("This prioritizes models with good generalization (positive CV)")
