...
# -*- coding: utf-8 -*-
"""
Final Dual Target Analysis with NEW BEST VARIABLE
G_GPD_PCAP_SLOPE + Score-Total tax and contribution rate (% of profit)
Uses balanced scoring (sqrt(R²) × sqrt(CV)) for model selection
"""
...
print("="*80)
print("FINAL DUAL TARGET ANALYSIS (NEW BEST VARIABLE)")
print("GDP Growth + Score-Total tax and contribution rate")
print("="*80)
...
targets = {
    ...
    'Tax_Score': 'Score-Total tax and contribution rate (% of profit)'
}
...
for target_name, target_col in targets.items():
    ...
    # Get predictors (excluding all G_ variables and new non-G variables)
    g_vars = [c for c in df.columns if c.startswith('G_')]

    # Identify columns from BASE_COMPLETA (new non-G variables)
    # These are columns that weren't in the original DATA_GHAB2
    exclude_cols = (['Pais', 'Etiqueta_pais', 'Country code', 'Country code2',
                     'Ease of Doing Business', 'New_Business_Density', 'Country', 'Economy'] +
                    g_vars)

    # For GDP Growth, exclude all Doing Business variables
    # For Tax Score, only exclude Tax-related variables
    if target_name == 'GDP_Growth':
        # Get all potential predictors from DATA_GHAB2 (original gender/development indicators)
        ...
        # For Tax Score, exclude tax-related and other Doing Business variables
        exclude_tax = [c for c in df.columns
                      if 'tax' in c.lower() or 'Tax' in c
                      or c.startswith('Score-') or c.startswith('Rank-')
                      or 'DB' in c or 'index' in c.lower()
                      or 'methodology' in c.lower()]
        potential_predictors = [col for col in df.columns
                               if col not in exclude_cols
                               and col not in exclude_tax]
...
# Save results
with open('resultados/modelos/final_dual_target_new_variable.pkl', 'wb') as f:
    pickle.dump(all_results, f)

print("\n" + "="*80)
print("SUMMARY (NEW VARIABLE - BALANCED SELECTION)")
print("="*80)
...
print(f"\nResults saved: resultados/modelos/final_dual_target_new_variable.pkl")
print("\nNote: Models selected using balanced score = sqrt(R²) × sqrt(CV)")
print("Ensures BOTH R² and CV are positive and adequate simultaneously")
