...
IMPROVED VERSION: Prioritizes models with POSITIVE CV and good generalization
"""
...
print("="*80)
print("ROBUST RECURSIVE G VARIABLE SELECTION")
print("Prioritizing positive CV and good generalization")
print("="*80)
...
print(f"Merged database: {df_merged.shape}")
...
print(f"\n{'='*80}")
print(f"G variables to test: {len(g_targets)}")
print(f"{'='*80}")
...
for target_var in g_targets:
    ...
    if len(correlations) < 5:
        print(f"WARNING: Insufficient correlations. Skipping.")
        continue
    ...
    if mask.sum() < 15:
        print(f"WARNING: Final dataset too small. Skipping.")
        continue
    ...
    try:
        ...
        print(f"    NN failed: {str(e)[:30]}")

    if len(models) == 0:
        print("WARNING: All models failed. Skipping.")
        continue

    # IMPROVED RANKING: Prioritize positive CV and good R²
    # Composite score: prioritize CV > 0, then balance R² and CV
    def get_composite_score(name, res):
        r2 = res['r2']
        cv = res['cv']

        # If CV is negative, penalize heavily
        if cv < 0:
            return r2 * 0.3  # Heavily penalized

        # If CV is positive, reward it
        # Composite = 0.6 * R² + 0.4 * CV (both positive)
        return 0.6 * max(0, r2) + 0.4 * cv

    ranked = sorted(models.items(),
                   key=lambda x: get_composite_score(x[0], x[1]),
                   reverse=True)

    best_name, best_res = ranked[0]

    print(f"\n  Best model: {best_name}")
    print(f"    R²={best_res['r2']:.4f}, CV={best_res['cv']:.4f}")
    print(f"    Composite score={get_composite_score(best_name, best_res):.4f}")
    ...
    # Store results with composite score
    all_g_results[target_var] = {
        ...
        'composite_score': get_composite_score(best_name, best_res)
    }

# Save results
with open('resultados/modelos/all_G_variables_results_robust.pkl', 'wb') as f:
    pickle.dump(all_g_results, f)

df_merged.to_excel('DATA_MERGED_COMPLETE.xlsx', index=False)

print("\n" + "="*80)
print("RANKING BY COMPOSITE SCORE (0.6*R² + 0.4*CV if CV>0)")
print("="*80)
...
print(f"\n{'Rank':<5} {'Variable':<50} {'R²':<8} {'CV':<8} {'Score':<8} {'N':<6} {'Model':<20}")
print("-"*110)

for i, (var, res) in enumerate(sorted_results, 1):
    best_name, best_res = res['best']
    var_short = var[:47] + '...' if len(var) > 47 else var
    print(f"{i:<5} {var_short:<50} {best_res['r2']:>6.3f}  {best_res['cv']:>6.3f}  {res['composite_score']:>6.3f}  {res['n_observations']:<6} {best_name:<20}")
...
if sorted_results:
    ...
    print(f"\n{'='*80}")
    print(f"BEST G VARIABLE (by composite score): {best_g_var}")
    print(f"{'='*80}")
    print(f"  Best Model: {best_model_name}")
    print(f"  R² Score: {best_model_res['r2']:.4f}")
    print(f"  CV R²: {best_model_res['cv']:.4f}")
    print(f"  Composite Score: {best_g_res['composite_score']:.4f}")
    ...
    # Check if CV is positive
    if best_model_res['cv'] > 0:
        print(f"\n  ✓ CV is POSITIVE - Good generalization!")
    else:
        print(f"\n  ⚠ Warning: CV is negative - May have overfitting issues")

print(f"\nResults saved: resultados/modelos/all_G_variables_results_robust.pkl")
